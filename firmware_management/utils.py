...
def get_status_for(status):
    ...
    if status_name not in dict(FirmwareStatusChoices):
        raise ImproperlyConfigured(
            f'netbox_inventory plugin configuration defines status {status_name}, but it is not defined in FIELD_CHOICES["netbox_inventory.Firmware.status"]'
        )
    return status_name
...
def get_all_statuses_for(status):
    ...
    if extra_statuses := status_names.difference(set(dict(FirmwareStatusChoices))):
        raise ImproperlyConfigured(
            f'netbox_inventory plugin configuration defines statuses {extra_statuses}, but these are not defined in FIELD_CHOICES["netbox_inventory.Firmware.status"]'
        )
